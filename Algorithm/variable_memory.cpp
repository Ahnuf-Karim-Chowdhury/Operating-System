...
public:
    VariableMemoryManager(const vector<int>& holes, const vector<int>& requests) {
        memoryHoles = holes;
    ...
    void runFirstFit() {
        ...
        for (int req : memoryRequests) {
            ...
            if (!allocated) cout << req << "\tNOT ALLOCATED\t";
        ...
        cout << "Internal Fragmentation: " << internalFragmentation << "\n";
        cout << "External Fragmentation: " << externalFragmentation << "\n";
    }
...
int main() {
    ...
    VariableMemoryManager manager(holes, requests);
    manager.runFirstFit();
    manager.runBestFit();
    manager.runWorstFit();
